...
class RNN(nn.Module):
    def __init__(self, in_size, h_size, num_layers, num_classes):
        ...
        self.rnn = nn.RNN(in_size, h_size, num_layers, batch_first = True)
        self.fc = nn.Linear(h_size * sequence_len, num_classes)
    
    def forward(self, x):
        h_0 = torch.zeros(self.num_layers, x.size(0), self.h_size).to(device) # first hidden layer
...
model = RNN(in_size, h_size, num_layers, num_classes).to(device)
